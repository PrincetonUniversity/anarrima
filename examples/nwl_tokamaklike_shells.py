...
from anarrima.reduced_integrals import (
    g_HA,
    g_HAa,
    g_HB,
    g_HBa,
...
g_HAa = jnp.vectorize(g_HAa)
g_VAa = jnp.vectorize(g_VAa)
g_HBa = jnp.vectorize(g_HBa)
g_VBa = jnp.vectorize(g_VBa)

plasma_data = np.loadtxt("desc_elliptical_equilibrium_b_field.csv", delimiter=",")
R, Z = plasma_data.T[0:2]
rho = plasma_data.T[2]
Br, Bphi, Bz = plasma_data.T[3:6]
dV = plasma_data.T[6]


# model a tokamak with a wide, flat fusion core
def density(rho):
    return 1 - rho**4
...
def fusion_proportion(rho):
    # n^2 <sigma v> and <sigma v> ~ T^2
    return density(rho) ** 2 * temperature(rho) ** 2


normalized_fusion = fusion_proportion(rho) * dV / sum(fusion_proportion(rho) * dV)


def alpha_beta(Br, Bphi, Bz):
    Bpol = jnp.sqrt(Br**2 + Bz**2)
    alpha = jnp.arctan2(-Bz, Br)
    beta = jnp.arctan2(Bpol, Bphi)
    return alpha, beta


alpha, beta = alpha_beta(Br, Bphi, Bz)
...
def wall_surface_areas(r, z):
    wr, _ = wall_centers(r, z)
    r0, r1, z0, z1 = _p0p1(r, z)
    dr = r1 - r0
    dz = z1 - z0
    norm = jnp.sqrt(1 + dz**2 / dr**2)
    seg = jnp.abs(np.pi * norm * (r0**2 - r1**2))
    return seg
...
def visible_angles(r, z, p):
    """
    r: array of wall vertices, r coords
    z: array of wall vertices, z coords
    p: plasma point (rp, zp)
    """
    rp, zp = p
...
def construct_nwl_fs_iso(g_H, g_V):
    def f(r, z, plasma_data):
        @jax.vmap
        def load_from_point(row):
            p, zp_abs, _, Br, Bphi, Bz, dV = row
            point = p, zp_abs

            cosϕ = visible_angles(r, z, point)
            ϕ = safe_arccos(cosϕ)
        ...
        return jnp.dot(normalized_fusion, load_from_point(plasma_data))

    return f


def construct_nwl_fs(g_H, g_V):
    def f(r, z, plasma_data):
        @jax.vmap
        def load_from_point(row):
            p, zp_abs, _, Br, Bphi, Bz, dV = row
            ...
            # g_HI: horizontal component, isotropic
            horiz = nr * g_H(p, zp_rel, wr, alpha, beta, ϕ)
            vert = nz * g_V(p, zp_rel, wr, alpha, beta, ϕ)
            return horiz + vert
...
nwl_A = construct_nwl_fs(g_HAa, g_VAa)
nwl_iso = construct_nwl_fs_iso(g_HI, g_VI)
nwl_B = construct_nwl_fs(g_HBa, g_VBa)
...
def objective_iso(x, desired_nwl):
    r, z = unpack_variables(x)
    nwl = (1 / 3) * nwl_iso(r, z, plasma_data)
...
loadings = np.round(np.geomspace(4.8, 7.0, num=5), 1)
...
for ax in axs:
    ellipse = Ellipse(
        (0.2, 0), width=0.05 * 2, height=0.075 * 2, facecolor="#AAAAAA33", zorder=2000
    )
    ...
    ax.quiver(
        R,
        Z,
        normalized_fusion * np.cos(alpha),
        -normalized_fusion * np.sin(alpha),
        scale=my_scale,
        width=0.005,
        pivot="mid",
        color="#333333",
...
axs[1].text(0.2, 0.14, r"4.8")
axs[1].text(0.252, 0.023, "7.0")

plt.savefig("nwl_tokamaklike_shells.pdf")
plt.show()
...
for wl, x in xs_A.items():
    axs[0].plot(
        θ,
        (1 / 2) * nwl_A(*unpack_variables(x, n_points=n_points_show), plasma_data),
        lw=0.5,
        color="red",
    )
...
for wl, x in xs_iso.items():
    axs[1].plot(
        θ,
        (1 / 3) * nwl_iso(*unpack_variables(x, n_points=n_points_show), plasma_data),
        lw=0.5,
        color="black",
    )
...
for wl, x in xs_B.items():
    axs[2].plot(
        θ,
        (2 / 3) * nwl_B(*unpack_variables(x, n_points=n_points_show), plasma_data),
        lw=0.5,
        color="blue",
    )
...
for ax in axs:
    ...
    ax.set_yscale("log")
    ax.set_yticks([4.8, 5.3, 5.8, 6.4, 7.0], labels=[4.8, 5.3, 5.8, 6.4, 7.0])
...
axs[1].set_title("Isotropic mode")
axs[2].set_title("B/C modes")
plt.savefig("nwl_tokamaklike_shells_verification.pdf")
